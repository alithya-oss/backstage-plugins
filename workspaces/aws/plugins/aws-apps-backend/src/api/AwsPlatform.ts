...
export class AwsAppsPlatformApi {
   ...
   *
   */
  public async getPlatformSecretValue(
    secretArn: string,
  ...
  public async deletePlatformSecret(
    ...
    this.logger.info(
      `Calling deletePlatformSecret for ${secretName} against platform region ${this.platformRegion}`,
    );
    const client = new SecretsManagerClient({
      region: this.platformRegion,
    });
  ...
  public async updateProvider(
    ...
    if (action === 'add') {
      ...
      newDependencies.push(
        `awsenvironmentprovider:default/${provider.name.toLowerCase()}`,
      );
