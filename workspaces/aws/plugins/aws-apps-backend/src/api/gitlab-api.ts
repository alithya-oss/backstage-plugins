...
export class GitLabAPI implements ISCMBackendAPI {
  ...
  private async getGitProjectId(
    ...
    const gitProjectsJson: { path_with_namespace: string; id: string }[] =
      await gitProjects.json();
  ...
  public async deleteRepository(
    ...
    if (!repo.projectID) {
      gitProjectId = await this.getGitProjectId(
        repo.gitHost,
        repo.gitProjectGroup || '',
        repo.gitRepoName,
        accessToken,
      );
      console.log(
        `Got GitLab project ID: ${gitProjectId} for ${repo.gitProjectGroup}/${repo.gitRepoName}`,
      );
    }
