...
`@alithya-oss/backstage-plugin-scaffolder-backend-aws-apps`
...
yarn add --cwd packages/backend @alithya-oss/backstage-plugin-scaffolder-backend-aws-apps@0.2.0
```
...
} from '@alithya-oss/backstage-plugin-scaffolder-backend-aws-apps';
...
The scaffolder actions which create AWS resources will leverage the AWS Environments and Environments Provider model provided in the `@alithya-oss/backstage-plugin-aws-apps-backend` plugin.
