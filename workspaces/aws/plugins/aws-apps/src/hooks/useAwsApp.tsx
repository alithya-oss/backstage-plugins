...
export const useAwsComponentFromContext = (): AwsComponentHookLoadingStatus => {
  ...
  async function getCatalogData(): Promise<EnvEntityMap> {
    ...
    const envRefs: EntityRelation[] = entity.relations.filter(
      relation => parseEntityRef(relation.targetRef).kind === 'awsenvironment',
    );
    if (!envRefs.length) {
      throw new Error(
        `Entity ${entity.metadata.name} does not have an AWS environment.`,
      );
    }
    ...
    const envEntities: EntityLookupResponse[] = await Promise.all(
      envRefs.map(
        async (relation: EntityRelation): Promise<EntityLookupResponse> => {
          const envEntity = await catalogApi.getEntityByRef(relation.targetRef);
          return {
            envName:
              envEntity?.metadata.name ||
              parseEntityRef(relation.targetRef).name,
    ...
    const envProviderRefs = envEntities
      ...
      .reduce(
        (
          acc: EntityNameAndRef[],
          envProviderEntities: MultiEntityNameAndRef,
        ) => {
    ...
    const envProviderEntityMap: EnvEntityMap = (
      await Promise.all(
        envProviderRefs.map(
          async (providerRef): Promise<EntityLookupResponse> => {
            const envProviderEntity = await catalogApi.getEntityByRef(
  ...
  async function populateServerlessState(
    svAppDeployEnv: AWSServerlessAppDeploymentEnvironment,
  ): Promise<void> {
    ...
    try {
      if (defaultDeployStatus === ExtraStackDeployStatus.STAGED) {
        const stackDetails = await api.getStackDetails({
          stackName: appStack.stackName,
        });
        // console.log("Stack Details");
        // console.log(JSON.stringify(stackDetails, null, 2));

        appStack.stackDeployStatus = (stackDetails.StackStatus ||
          ExtraStackDeployStatus.STAGED) as DeployStackStatus;
        appStack.creationTime = stackDetails.CreationTime
          ? formatWithTime(new Date(stackDetails.CreationTime))
          : undefined;
        appStack.lastUpdatedTime = stackDetails.LastUpdatedTime
          ? formatWithTime(new Date(stackDetails.LastUpdatedTime))
          : undefined;

        if (stackDetails.Outputs) {
          const logGroupsArrayOutput =
            stackDetails.Outputs.filter(
              output => output.OutputKey === 'LogGroupsArray',
            )[0] ?? null;
          if (logGroupsArrayOutput) {
            svAppDeployEnv.app.logGroupNames.push(
              ...JSON.parse(logGroupsArrayOutput.OutputValue as string),
            );
          }

          const apiUrlOutput =
            stackDetails.Outputs.filter(
              output => output.OutputKey === 'ApiGatewayEndpointWithPath',
            )[0] ?? null;
          if (apiUrlOutput) {
  ...
  function populateEcsState(
    ecsAppDeployEnv: AWSECSAppDeploymentEnvironment,
    providerEntity: Entity,
  ): void {
    ecsAppDeployEnv.clusterName =
      providerEntity.metadata.clusterName?.toString() || '';
  ...
  function populateEksState(
    eksAppDeployEnv: AWSEKSAppDeploymentEnvironment,
    providerEntity: Entity,
  ): void {
    eksAppDeployEnv.clusterName =
      providerEntity.metadata.clusterName?.toString() || '';
    ...
    eksAppDeployEnv.app = {
      ...
      logGroupName:
        providerAppData.LogGroup ||
        `/aws/apps/${eksAppDeployEnv.providerData.prefix}-${eksAppDeployEnv.providerData.name}/${providerAppData.Namespace}`,
  ...
  function populateResourceState(
    resourceDeployEnv: AWSResourceDeploymentEnvironment,
  ): void {
    ...
    resourceDeployEnv.resource.resourceGroupArn =
      providerAppData.ResourceGroup?.toString() || '';
    resourceDeployEnv.resource.cloudFormationStackName =
      providerAppData.StackName?.toString() || '';
  }
  ...
  async function getData(): Promise<AWSComponent> {
    ...
    const deployEnvs: AwsDeploymentEnvironments = envEntities.reduce(
      (acc, envEntity) => {
        const envProviders = envProviderEntityMap[envEntity.envName];
        let envProvider;

        if (envEntity.envName === _change_to_env_name) {
          envProvider = envProviders.filter(
            (providerEntity: Entity) =>
              providerEntity.metadata.name === _change_to_env_provider_name,
          )[0];
        ...
        const awsDeploymentEnvironment: AWSDeploymentEnvironment = {
          environment: {
            accountType:
              envEntity.entity?.metadata.envTypeAccount?.toString() || '',
            category: envEntity.entity?.metadata.category?.toString() || '',
            classification:
              envEntity.entity?.metadata.classification?.toString() || '',
            description:
              envEntity.entity?.metadata.description?.toString() || '',
            envType:
              envEntity.entity?.metadata.environmentType?.toString() || '',
            level: parseInt(
              envEntity.entity?.metadata.level?.toString() || '0',
              10,
            ),
            name: envEntity.entity?.metadata.name.toString() || '',
            regionType:
              envEntity.entity?.metadata.envTypeRegion?.toString() || '',
          },
          providerData: {
            ...
            operationRoleSsmKey:
              envProvider.metadata.operationRole?.toString() || '',
            provisioningRoleSsmKey:
              envProvider.metadata.provisioningRole?.toString() || '',
            providerType:
              envProvider.metadata.envType?.toString().toLowerCase() || '',
            vpcSsmKey: envProvider.metadata.vpc?.toString() || '',
            cloudFormationStackName:
              envProvider.metadata.StackName?.toString() || '',
            terraformWorkspace:
              envProvider.metadata.TerraformWorkspace?.toString() || '',
            terraformStateBucket:
              envProvider.metadata.TerraformStateBucket?.toString() || '',
            terraformStateTable:
              envProvider.metadata.TerraformStateTable?.toString() || '',
          },
          entities: {
        ...
        const providerType =
          envEntity.entity?.metadata.environmentType
            ?.toString()
            .toLowerCase() || 'N/A';

        if (providerType === 'N/A') {
          throw new Error(
            'Environment Entity not set properly - please configure environmentType',
          );
        }

        if (providerType === ProviderType.ECS && componentType === 'aws-app') {
          populateEcsState(
            awsDeploymentEnvironment as AWSECSAppDeploymentEnvironment,
            envProvider,
          );
        } else if (
          providerType === ProviderType.EKS &&
          componentType === 'aws-app'
        ) {
          populateEksState(
            awsDeploymentEnvironment as AWSEKSAppDeploymentEnvironment,
            envProvider,
          );
        } else if (
          providerType === ProviderType.SERVERLESS &&
          componentType === 'aws-app'
        ) {
          // Must handle this later, since it will require async calls that cannot be made here
        } else if (componentType === 'aws-resource') {
          populateResourceState(
            awsDeploymentEnvironment as AWSResourceDeploymentEnvironment,
          );
    ...
    function getLowerEnvironment(
      envs: AwsDeploymentEnvironments,
    ): GenericAWSEnvironment {
    ...
    const awsComponent: AWSComponent = {
      ...
      currentEnvironment: !!_change_to_env_name
        ? deployEnvs[_change_to_env_name.toLowerCase()]
        : getLowerEnvironment(deployEnvs),
    ...
    if (
      ...
      await populateServerlessState(
        awsComponent.currentEnvironment as AWSServerlessAppDeploymentEnvironment,
      );
    }
    if (
      awsComponent.currentEnvironment.providerData.providerType ===
        ProviderType.GENAI_SERVERLESS &&
      componentType === 'aws-app'
    ) {
  ...
  const {
    ...
    loading,
    retry: refresh,
  } = useAsyncRetry(() => getData(), [catalogApi, entity]);
...
const AwsAppContext = createContext<AwsComponentHookLoadingStatus>({
  loading: true,
});
...
export const AsyncAwsAppProvider = (props: AsyncAwsAppProviderProps) => {
  ...
  return (
    <AwsAppContext.Provider value={value}>{children}</AwsAppContext.Provider>
  );
};
