...
const ResourceBindingCard = ({
  ...
  async function bindResource(item: ResourceBinding): Promise<any> {
    ...
    if (item.resourceType === 'aws-rds') {
      ...
      if (item.associatedResources && item.associatedResources.length > 0) {
        item.associatedResources.forEach(ar => {
          if (ar.resourceType === 'aws-db-secret') {
            const secretPolicy = SECRET_POLICY.replace(
              '@@@PLACEHOLDER@@@',
              ar.resourceArn,
            );
            policies.push({
      ...
      const s3Policy = S3_POLICY.replace(
        '@@@PLACEHOLDER@@@',
        `"${item.resourceArn}","${item.resourceArn}/*"`,
      );
      policies.push({
      ...
      <CardContent>
            ...
            {items.map((record, index) => {
              return (
                <TableRow key={record.id}>
                  ...
                  <TableCell id="provider">
                    <EntityRefLink
                      entityRef={`awsenvironmentprovider:default/${record.provider}`}
                      title={record.provider}
                    />
                  </TableCell>
                  <TableCell id="resourceName">
                    <EntityRefLink
        ...
        <Typography margin="10px">
          ...
          <ResourceSelectorDialog
            associatedResources={items}
            currentEnvironment={
              awsComponent.currentEnvironment.environment.name
            }
            catalog={catalog}
            isOpen={openDialog}
            selectHandler={selectResourceHandler}
