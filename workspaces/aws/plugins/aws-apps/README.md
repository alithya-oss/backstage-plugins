...
# AWS Apps on AWS Frontend

This is the frontend UI plugin for the the AWS Apps on AWS plugin set. An AWS Catalog Page, AWS Apps Homepage and several entity cards are contributed to the UI from this plugin.
...
This is for manual installation, if you install the AWS Apps platform as a complete suite - git patch will take care of the below configuration.

```sh
# From your Backstage root directory
yarn add --cwd packages/app @alithya-oss/backstage-plugin-aws-apps@0.4.0
```

## Configuration

Each of the UI components contributed in the AWS Apps on AWS frontend plugin can be configured independently and added to your Backstage platform as desired. Details for adding each type of UI component are found in the sections below.
...
+import { AwsAppsHomePage } from '@alithya-oss/backstage-plugin-aws-apps/src/demo/AwsAppsHomePage/AwsAppsHomePage';
...
This backstage backend plugin depends on other AWS Apps plugins - in order to compile the code properly, make sure all dependencies are installed and configured properly.
