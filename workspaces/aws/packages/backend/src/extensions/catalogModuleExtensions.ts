...
import {
  AWSEnvironmentEntitiesProcessor,
  AWSEnvironmentProviderEntitiesProcessor,
} from '@alithya-oss/plugin-catalog-backend-module-aws-apps-entities-processor';
    ...
    env.registerInit({
      ...
      async init({ catalog, config, logger }) {
        ...
        catalog.addProcessor(
          await AwsOrganizationCloudAccountProcessor.fromConfig(config, {
            logger: logger,
          }),
        );
