 ...
 */
export class TimeSaverDatabase {
  readonly #database: PluginDatabaseManager;
  #promise: Promise<Knex> | undefined;

  static create(database: PluginDatabaseManager): TimeSaverDatabase {
    return new TimeSaverDatabase(database);
  }

  /** @internal */
  static forTesting(): TimeSaverDatabase {
  ...
  get(): Promise<Knex> {
    this.#promise ??= this.#database.getClient().then(async client => {
      if (!this.#database.migrations?.skip) {
        await TimeSaverDatabase.runMigrations(client);
      }
      return client;
    });
