...
const mockManager = {
  ...
  getLifecycle: () => mockServices.rootLifecycle.mock(),
};

const logger = mockManager.getLogger();
const lifecycle = mockManager.getLifecycle();
...
describe('TimeSaverDatabase', () => {
  ...
  describe('getTemplateNameByTemplateTaskId', () => {
    beforeAll(async () => {
    ...
    it('returns template name string from row matching TemplateTaskId', async () => {
      const result = await database.getTemplateNameByTemplateTaskId(
  ...
  describe('getDailyTimeSummariesByTeam', () => {
    ...
    it('returns total time saved by each team each year, in date ascending order', async () => {
      const result = await database.getDailyTimeSummariesByTeam();
  ...
  describe('getDailyTimeSummariesByTemplate', () => {
    ...
    it('returns total time saved by each template each year, in date ascending order', async () => {
      const result = await database.getDailyTimeSummariesByTemplate();
      // Only take the year into account to re-create date values
      expect(result).toStrictEqual([
        {
          date: DateTime.fromISO('2024-07-17T00:00:00'),
  ...
  describe('getTimeSavedSummaryByTeam', () => {
    ...
    it('returns the total time saved per team per date', async () => {
      const result = await database.getTimeSavedSummaryByTeam();
      // Only take the year into account to re-create date values
      expect(result).toStrictEqual([
        {
          date: DateTime.fromISO('2024-07-17T00:00:00'),
  ...
  describe('getTimeSavedSummaryByTemplate', () => {
    ...
    it('...', async () => {
      const result = await database.getTimeSavedSummaryByTemplate();
      // Only take the year into account to re-create date values
      expect(result).toStrictEqual([
        {
          date: DateTime.fromISO('2024-07-17T00:00:00'),
