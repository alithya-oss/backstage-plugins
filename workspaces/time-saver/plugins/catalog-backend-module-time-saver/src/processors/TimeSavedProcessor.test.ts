...
import { TimeSavedProcessor } from "./TimeSavedProcessor";

function copy(entity: Entity): Entity {
  return JSON.parse(JSON.stringify(entity));
}

describe("TimeSavedProcessor", () => {
  let processorUnderTest: TimeSavedProcessor;

  beforeEach(() => {
    processorUnderTest = new TimeSavedProcessor();
  });

  it("returns expected processor name", () => {
    expect(processorUnderTest.getProcessorName()).toEqual("TimeSavedProcessor");
  });
