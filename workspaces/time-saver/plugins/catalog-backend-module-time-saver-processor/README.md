...
1. Install the plugin package in your Backstage backend:
   ...
   yarn workspace packages/backend add @alithya-oss/backstage-plugin-catalog-backend-module-time-saver-processor
   ```
...
2. Wire up the processor in your backend.
   ...
   + import {
   +   TimeSaverProcessor,
   + } from '@alithya-oss/backstage-plugin-catalog-backend-module-time-saver-processor';
   ...
   **New Backend System**
   ...
   + backend.add(import('@alithya-oss/backstage-plugin-catalog-backend-module-time-saver-processor'));
   // ...
   backend.start();
   ```
