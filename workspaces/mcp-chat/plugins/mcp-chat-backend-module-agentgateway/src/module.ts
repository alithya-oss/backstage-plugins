  ...
  register(reg) {
    reg.registerInit({
      ...
      async init({ config, llmProviders }) {
        ...
        const entry = providers.find(p => p.getString('id') === 'agentgateway');
        ...
        const providerConfig = {
          ...
          baseUrl:
            entry.getOptionalString('baseUrl') || 'http://localhost:5555/v1',
        ...
        llmProviders.registerProvider(
          'agentgateway',
          new AgentGatewayProvider(providerConfig),
        );
      },
    });
  },
