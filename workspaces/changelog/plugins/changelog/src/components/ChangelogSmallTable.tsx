...
const ChangelogActionsCounters = ({
  changelogActions,
}: {
  changelogActions: ChangelogAction[];
}) => {
  return (
    <>
      {changelogActions.map(changelogAction => {
        if (changelogAction.counter) {
          return (
            <ActionCounter
              actionName={changelogAction.name}
              counter={changelogAction.counter}
              key={changelogAction.name}
              Icon={changelogAction.icon}
            />
          );
        }
        return null;
...
export const ChangelogSmallTable = ({
  changelogInfos,
}: {
  ...
  const columns: TableColumn[] = [
    ...
    },
    {
      ...
      customFilterAndSearch: (filter: string, rowData: any, _) => {
        if (rowData.actions.length) {
          return (
            rowData.actions.find((action: { name: string }) =>
    ...
    },
    {
      ...
      render: (changelogInfo: any) => {
        return (
          <>
            {changelogInfo.versionContent && (
              <SeeChangesButton contentMd={changelogInfo.versionContent} />
            )}
          </>
        );
      },
